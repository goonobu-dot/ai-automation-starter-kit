...
def test_operator_can_replace_ai_selected_frame_without_losing_answers(tmp_path):
    workspace = tmp_path / "manual-workspace"
    create_manual_studio(workspace, name="Invoice entry", language="en")
    (workspace / "01_RECORDINGS" / "invoice-entry.mp4").write_bytes(b"safe-fixture-video")
    ffmpeg, ffprobe = _fake_media_tools(tmp_path)
    ...
    answer_manual_question(
        ...
        source_reference="Process owner confirmation",
        answered_by="Process owner",
    )

    payload = apply_manual_frame_selections(
        workspace,
        selections=[{"order": 1, "frame_path": frames[0]["path"]}],
        selected_by="Local operator",
    )

    assert payload["changed"] == 1
    assert payload["manual"].endswith("04_DRAFT_MANUAL/manual.html")
    ...
    assert plan["steps"][0]["timestamp_seconds"] == frames[0]["timestamp_seconds"]
    manual = (workspace / "04_DRAFT_MANUAL" / "manual.html").read_text(encoding="utf-8")
    assert frames[0]["path"] in manual
    assert frames[1]["path"] not in manual
    ledger = json.loads((workspace / "05_REVIEW" / "answers.json").read_text(encoding="utf-8"))
    assert ledger["answers"][0]["answer"] == "Finance manager"
    assert ledger["draft_plan_sha256"] == manual_studio_module._sha256(
        workspace / "04_DRAFT_MANUAL" / "manual_plan.json"
    )
    history = json.loads((workspace / ".system" / "frame_selection_history.json").read_text(encoding="utf-8"))
    assert history["changes"][-1]["selected_by"] == "Local operator"
    assert history["changes"][-1]["old_frame_path"] == frames[1]["path"]
    assert history["changes"][-1]["new_frame_path"] == frames[0]["path"]


def test_operator_frame_selection_rejects_unknown_or_cross_video_frame(tmp_path):
    workspace = tmp_path / "manual-workspace"
    create_manual_studio(workspace, name="Two recordings", language="en")
    for name in ("first.mp4", "second.mp4"):
        (workspace / "01_RECORDINGS" / name).write_bytes((name + "-fixture").encode("utf-8"))
    ffmpeg, ffprobe = _fake_media_tools(tmp_path)
    prepare_manual_recordings(workspace, ffmpeg=str(ffmpeg), ffprobe=str(ffprobe))
    manifest = json.loads((workspace / ".system" / "capture_manifest.json").read_text(encoding="utf-8"))
    first_frame = manifest["videos"][0]["frames"][0]
    second_frame = manifest["videos"][1]["frames"][0]
    draft = {
        "title": "Two recordings",
        "summary": "Use the first recording.",
        "steps": [
            {
                "order": 1,
                "title": "First step",
                "instruction": "Follow the first recording.",
                "frame_path": first_frame["path"],
                "source_video": manifest["videos"][0]["path"],
    ...
    build_manual_with_codex(workspace, executable=str(_fake_codex(tmp_path, draft)), title="Two recordings")

    with pytest.raises(ValueError, match="same source video"):
        apply_manual_frame_selections(
            workspace,
            selections=[{"order": 1, "frame_path": second_frame["path"]}],
        )
    with pytest.raises(ValueError, match="capture manifest"):
        apply_manual_frame_selections(
            workspace,
            selections=[{"order": 1, "frame_path": "02_EXTRACTED_FRAMES/missing.png"}],
        )
...
def test_manual_studio_beginner_guides_are_bilingual_and_concrete():
    ...
    assert "日本語で一つずつ案内してください" in japanese
    assert "ターミナルへコマンドを入力する必要はありません" in japanese
    assert "<pre" not in japanese
    assert "ai-automation-kit manual-studio" not in japanese
    assert "Add a recording" in english
    assert "screenshots" in english
    assert "one question at a time" in english
    assert "guide me one step at a time" in english
    assert "You do not need to type terminal commands" in english
    assert "<pre" not in english
    assert "ai-automation-kit manual-studio" not in english
