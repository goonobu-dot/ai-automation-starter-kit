...
- ターミナル（文字でパソコンに指示を出す画面のことです。以下で開き方を説明します）

## ターミナルの開き方・使い方（初めての方向け）

「ターミナル」は、普段使っているアプリのアイコンをクリックする代わりに、文字を打ち込んでパソコンに指示を出す画面です。真っ黒（または白）な画面に文字だけが表示されます。最初は不安に思うかもしれませんが、やることは「決まった文字をコピペしてEnterキーを押す」だけです。

### Mac の場合

1. 画面右上の虫眼鏡マーク（Spotlight検索）をクリックするか、`command（⌘）キー + スペースキー` を同時に押します。
2. 出てきた検索欄に「ターミナル」と入力します。
3. 「ターミナル.app」または「Terminal」という候補が出るのでクリック（またはEnter）します。
4. 黒っぽい画面が開けば成功です。これが「ターミナル」です。

（もう一つの方法: 「Finder」→「アプリケーション」→「ユーティリティ」フォルダの中に「ターミナル.app」があります）

### Windows の場合

1. 画面左下の「スタート」ボタンをクリックするか、`Windowsキー` を押します。
2. 「PowerShell」と入力します。
3. 「Windows PowerShell」という候補が出るのでクリックします（管理者権限は不要です。「管理者として実行」を選ぶ必要はありません）。
4. 青っぽい画面が開けば成功です。

### ターミナルの基本の使い方

- 画面に文字（プロンプトと呼びます）が表示され、その右側にカーソルが点滅しています。ここにコマンド（指示文）を入力します。
- このマニュアルに出てくる灰色の枠で囲まれたコード（例: `python3 --version`）は、そのままコピーして、ターミナルの画面上で右クリック→「貼り付け（ペースト）」するか、`command（⌘）+ V`（Windowsは `Ctrl + V`）で貼り付けられます。
- 貼り付けただけではまだ実行されません。**必ず `Enter` キーを押してください。** Enterを押した瞬間にコマンドが実行されます。
- 複数行のコードをまとめてコピペした場合は、1行ずつ自動的に実行されます（1行ごとにEnterを押す必要はありません）。
- 実行が終わると、次のプロンプト（入力待ちの状態）が表示されます。何も表示されずに次の入力待ちに戻っていれば「成功」です。
- 赤い文字や `error`、`command not found`、`No such file or directory` のような英語が表示されたら「失敗」です。エラーの文章をそのまま [FAQ](FAQ.ja.md) で検索するか、AIエージェント（ChatGPTやClaudeなど）にそのままコピペして「このエラーの意味と直し方を教えて」と聞くのが一番早い解決方法です。
- `cd フォルダ名` というコマンドは「そのフォルダの中に移動する」という意味です（cd = change directory）。このマニュアルの手順どおりに実行すれば、今どこにいるか迷うことはありません。
- 一度開いたターミナルのウィンドウは、作業の間は閉じずに開いたままにしておいてください。閉じてしまっても、また同じ手順で開き直せば大丈夫です。
