...
このページは、AIやクラウドにまだ慣れていない人が、AI Automation Starter Kit の役割を安心して理解するためのページです。
...
収益を保証するものではありません。企業の困りごとを見つけ、無理のない dry-run や小さなPoCとして説明しやすくするための道具です。
...
まず、全体像を知りたい場合は [はじめかた](GETTING_STARTED.ja.md) を読みます。

副業や受託として最初の1件を進めたい場合は [最初の顧客案件ウォークスルー](FIRST_CLIENT_WALKTHROUGH.ja.md) を読みます。

クラウド、APIキー、受付フォルダ、承認者などが不安な場合は [AI初心者サポートマップ](archive/AI_BEGINNER_SUPPORT_MAP.ja.md) をAIエージェントに読ませて、1問ずつ案内してもらいます。

実際に試す場合は、プロジェクトをダウンロードします。
...
最初の流れをまとめて実行します。迷った場合は `complete-workspace` から始めると、見本フォルダ、デモ、レポート、事業化提案資料までまとめて作れます。
...
生成されるファイルは多いですが、最初から全部読む必要はありません。まず「入口」「デモ」「レポート」「事業化提案」の4つを見れば大丈夫です。
