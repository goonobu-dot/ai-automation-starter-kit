...
1. Read the [beginner guide](BEGINNER_GUIDE.md) to understand the purpose.
2. Read the first sections of the [user manual](USER_MANUAL.md): safety, first path, and first workspace.
3. If cloud, APIs, or approvals feel confusing, ask an AI agent to read [AI Beginner Support Map](AI_BEGINNER_SUPPORT_MAP.md) with you.
4. If you want to turn this into a freelance or consulting offer, read [First Client Walkthrough](FIRST_CLIENT_WALKTHROUGH.md).
5. After local setup, run `ai-automation-kit complete-workspace --flow-id invoice-document-followup --client-type local-business --niche accounting --output .tmp/complete-accounting`.
6. Open `.tmp/complete-accounting/client_command_center.html` and `.tmp/complete-accounting/FINAL_DELIVERY_GUIDE.md`.
7. Before moving into APIs or cloud, use `guided-setup`, `guided-review`, and `cloud-plan` to list missing inputs and human approvals.
