...
Use strict monthly IDs matching `^[0-9]{4}-(0[1-9]|1[0-2])$`. Write mutable state JSON atomically with a temporary regular file, `fsync`, and `os.replace`. Publish collision-safe immutable JSON artifacts from a temporary regular file after `fsync` using an atomic hard link, or an equivalent no-overwrite primitive, so an existing or raced destination is never overwritten. Reject symlinked workspace components. Phase 1A workspace mutation requires macOS or Linux with `O_NOFOLLOW` and the required directory-relative filesystem operations; unsupported platforms fail before creating or changing a workspace, while unrelated package features remain available.

Derive the PIN with `hashlib.scrypt(pin.encode(), salt=16 random bytes, n=2**14, r=8, p=1)` when available. If that API is absent, use only an already-available, audited in-process `cryptography` Scrypt provider with byte-equivalent parameters and no new project dependency. If neither in-process provider is available, fail closed with guidance to use a supported Python runtime. Never place the PIN in process arguments, environment variables, logs, files, or exception text. Compare derived hashes with `hmac.compare_digest`.

Before loading approval-relevant workspace, period, or audit state, `approve_draft()` atomically creates `.system/approval.lock` with no-follow protection and holds it through approved artifact publication, audit append, and both state saves. A concurrent approval fails without publishing. Lock symlinks are rejected. A regular lock left after an interrupted process is never removed automatically: the operator must first confirm no approval process is running, then remove `.system/approval.lock` manually and retry.
