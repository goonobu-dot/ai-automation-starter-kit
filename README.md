...
For selling and choosing work, `opportunity-catalog` creates a browser-friendly automation offer catalog, `recommend-flow` turns a short client intake into a recommended flow and PoC scope, `guided-setup` creates a step-by-step input checklist for local or cloud setup, and `share-check` scans generated assets before sharing them with a client.
