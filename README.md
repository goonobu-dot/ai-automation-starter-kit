...
Prefer a browser page? Open the [English HTML manual](docs/manual.html) or [日本語HTMLマニュアル](docs/manual.ja.html). They give a visual entrance, route cards, a workflow map, and safety boundaries before you read the detailed manuals.

Do not read everything first. 最初から全部読まないでください。Start with the [English HTML manual](docs/manual.html), [日本語HTMLマニュアル](docs/manual.ja.html), [Beginner Route Map](docs/BEGINNER_ROUTE_MAP.md), or [初心者ルートマップ](docs/BEGINNER_ROUTE_MAP.ja.md). It tells you which files to open, which files to ignore at first, and which route to choose: No-CLI, CLI, side-hustle, company internal, website project, or cloud/API setup.
...
- [English HTML manual](docs/manual.html)
- [日本語HTMLマニュアル](docs/manual.ja.html)
...
- [AI beginner support map](docs/AI_BEGINNER_SUPPORT_MAP.md)
- [AI初心者サポートマップ](docs/AI_BEGINNER_SUPPORT_MAP.ja.md)
- [AI agent setup assistant playbook](docs/AI_AGENT_SETUP_ASSISTANT_PLAYBOOK.md)
- [AIエージェント セットアップ補助プレイブック](docs/AI_AGENT_SETUP_ASSISTANT_PLAYBOOK.ja.md)
- [Client delivery support runbook](docs/CLIENT_DELIVERY_SUPPORT_RUNBOOK.md)
- [顧客導入サポート運用手順書](docs/CLIENT_DELIVERY_SUPPORT_RUNBOOK.ja.md)
...
| `cloud_provider_matrix.md` | Provider, workload, runtime, service, and beginner-fit decision table. |
| `workload_architecture.md` | Architecture for webhook/API, scheduled job, worker queue, web app, static functions, or container service. |
| `runtime_choice.md` | Explanation of why the selected runtime fits the workload and when to change it. |
| `secrets_and_env.md` | Provider-specific secret and environment variable plan for selected connectors. |
| `network_and_domain.md` | Public/private endpoint, domain, DNS, and webhook activation notes. |
| `deploy_runbook.md` | Review-first CLI/dashboard deployment commands for the selected provider. |
| `operations_runbook.md` | Safe post-deploy checks for logs, dry-run behavior, test events, and stop conditions. |
| `cost_guardrails.md` | Beginner cost, budget alert, billing owner, and usage review notes. |
| `compliance_data_boundary.md` | Data boundary decisions before logs, prompts, queues, or exports touch cloud services. |
| `incident_rollback.md` | Stop traffic, disable scheduler/queue/webhook, rotate secrets, preserve logs, and notify owner. |
...
| `START_HERE_SIDE_HUSTLE_BLUEPRINTS.md` | Start guide generated by `side-hustle-blueprints` for choosing a sellable automation offer. |
| `side_hustle_blueprints.md` | Catalog of sellable automation blueprints with buyer, pain, inputs, deliverables, tools, pricing, and approval boundary. |
| `side_hustle_blueprints.html` | Browser-friendly catalog for quickly scanning offers with a client or AI agent. |
| `first_client_picker.md` | Scoring sheet for choosing the easiest first client and safest first offer. |
...
Generate the side-hustle blueprint catalog when you want to choose which automation offer to sell first:
...
- `.tmp/side-hustle-blueprints/START_HERE_SIDE_HUSTLE_BLUEPRINTS.md`
- `.tmp/side-hustle-blueprints/side_hustle_blueprints.html`
- `.tmp/side-hustle-blueprints/first_client_picker.md`
- `.tmp/side-hustle-blueprints/implementation_paths.md`
- `.tmp/side-hustle-blueprints/ai_agent_handoff.md`
...
Generate a catalog of sellable automation side-hustle offers before choosing a client or writing code:
...
Generate a provider-specific cloud deployment plan for a selected flow. This is cloud-wide support, not a LINE Bot-only helper.

```bash
ai-automation-kit cloud-plan --flow-id invoice-document-followup --provider aws --workload scheduled-job --connectors gmail,google-sheets --output .tmp/cloud-plan-aws
ai-automation-kit cloud-plan --flow-id support-reply-draft --provider google-cloud --workload webhook-api --connectors webhook,google-sheets --output .tmp/cloud-plan-gcp
ai-automation-kit cloud-plan --flow-id weekly-kpi-report --provider azure --workload worker-queue --connectors teams,storage-folder --output .tmp/cloud-plan-azure
ai-automation-kit cloud-plan --flow-id crm-lead-cleanup --provider render --workload web-app --connectors crm,gmail --output .tmp/cloud-plan-render
ai-automation-kit cloud-plan --flow-id appointment-reminder --provider railway --workload scheduled-job --connectors email,google-sheets --output .tmp/cloud-plan-railway
ai-automation-kit cloud-plan --flow-id order-shipping-status --provider vercel --workload static-functions --connectors webhook --output .tmp/cloud-plan-vercel
ai-automation-kit cloud-plan --flow-id inventory-reorder-alert --provider digitalocean --workload container-service --connectors email,storage-folder --output .tmp/cloud-plan-digitalocean
ai-automation-kit cloud-plan --flow-id field-service-dispatch --provider fly --workload container-service --connectors slack,storage-folder --output .tmp/cloud-plan-fly
```

Supported workloads are `webhook-api`, `scheduled-job`, `worker-queue`, `web-app`, `static-functions`, and `container-service`. Connectors such as `line`, `gmail`, `google-sheets`, `slack`, `teams`, `crm`, `webhook`, `email`, `storage-folder`, or `local-folder` only change the secret/setup checklist; they do not make the plan specific to one example.
...
Open `docs/manual.html` or `docs/manual.ja.html` in a browser for the human-first visual manual. Open `docs/demo.html` for a static demo page that shows the value story and generated artifacts.
